let express,jwtAuthenticated,getCurrentUser,Evento,User,Jwt;_ccf‍.x([["default",()=>_ccf‍.o]]);_ccf‍.w("express",[["default",["express"],function(v){express=v}]]);_ccf‍.w("../helpers/jwtAuthenticated",[["default",["jwtAuthenticated"],function(v){jwtAuthenticated=v}]]);_ccf‍.w("../helpers/getCurrentUser",[["default",["getCurrentUser"],function(v){getCurrentUser=v}]]);_ccf‍.w("../models/Event.js",[["default",["Evento"],function(v){Evento=v}]]);_ccf‍.w("../models/user.js",[["default",["User"],function(v){User=v}]]);_ccf‍.w("jsonwebtoken",[["default",["Jwt"],function(v){Jwt=v}]]);
...
router.post("/eventos/actualizarEstado", async (req, res) => {
  try {
    // Buscar cualquier propiedad booleana en el cuerpo de la solicitud
    const estado = Object.values(req.body).find(value => typeof value === 'boolean');

    // Verificar que se proporciona el nuevo estado
    if (estado === undefined) {
    ...
    res.status(200).json({ mensaje: `Estado de todos los eventos actualizado a ${estado}` });
  } catch (error) {
    _ccf‍.g.console.error(error);
...
router.post("/usuarios/crear", async (req, res) => {
  try {
    ...
    _ccf‍.g.console.error("Error al crear usuario:", error);
    res.send(JSON.stringify({
      success: false,
